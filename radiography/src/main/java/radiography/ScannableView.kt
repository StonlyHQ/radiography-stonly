...
public sealed class ScannableView {
    ...
    public class ComposeView(
        ...
        public val parent: ScannableView,
        public val layoutInfo: ComposeLayoutInfo
    ) : ScannableView() {
        override fun toString(): String = when (layoutInfo) {
            is AndroidViewInfo -> "${ComposeView::class.java.simpleName}/$displayName/${layoutInfo.view.toString()}"
            is LayoutNodeInfo -> "${ComposeView::class.java.simpleName}/$displayName/${layoutInfo.name}"
            is SubcompositionInfo -> "${ComposeView::class.java.simpleName}/$displayName/${layoutInfo.name}"
        }
    }
...
@OptIn(ExperimentalRadiographyComposeApi::class)
internal fun ComposeLayoutInfo.toScannableView(parent: ScannableView): ScannableView = when (val layoutInfo = this) {
    is LayoutNodeInfo -> ComposeView(
        ...
        children = layoutInfo.children.map { it.toScannableView(parent) },
        parent = parent,
